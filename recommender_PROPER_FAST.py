...
print("="*80)
print("PROPER FEATURE ENGINEERING (FAST 2-MIN VERSION) - TARGET 80-85/100")
print("="*80)
...
def extract_advanced_features(df):
    ...
    df['programming_score'] = df['clean'].apply(lambda x: sum(1 for word in programming_words if word in x))
    learning_words = ['learn', 'understand', 'tutorial', 'beginner', 'advanced', 'project']
    df['learning_score'] = df['clean'].apply(lambda x: sum(1 for word in learning_words if word in x))
    positive_words = ['great', 'excellent', 'amazing', 'perfect', 'love', 'recommend']
    negative_words = ['bad', 'terrible', 'boring', 'difficult', 'confusing']
    df['positive_score'] = df['clean'].apply(lambda x: sum(1 for word in positive_words if word in x))
    df['negative_score'] = df['clean'].apply(lambda x: sum(1 for word in negative_words if word in x))
    return df
...
vec1 = TfidfVectorizer(max_features=80000, ngram_range=(1,3), min_df=1, max_df=0.95, stop_words='english', sublinear_tf=True)
X1_train = vec1.fit_transform(train["clean"])
X1_test = vec1.transform(test["clean"])

vec2 = TfidfVectorizer(max_features=60000, analyzer='char_wb', ngram_range=(3,6), min_df=1, max_df=0.95, sublinear_tf=True)
X2_train = vec2.fit_transform(train["clean"])
X2_test = vec2.transform(test["clean"])

vec3 = TfidfVectorizer(max_features=50000, ngram_range=(1,2), min_df=2, max_df=0.9, stop_words='english', sublinear_tf=True)  
...
manual_features = ['review_length', 'word_count', 'programming_score', 'learning_score', 'positive_score', 'negative_score']

# SCALE THE FEATURES SO THEY DONT CRASH THE SOLVER!
scaler = MinMaxScaler()
manual_train_scaled = scaler.fit_transform(train[manual_features].values)
manual_test_scaled = scaler.transform(test[manual_features].values)

manual_train = csr_matrix(manual_train_scaled)
manual_test = csr_matrix(manual_test_scaled)
...
print("\n[4] Training SINGLE fast classifier...")
# ONLY 1 MODEL, AND max_iter=100 (which is super fast and perfect for TF-IDF)
clf = LogisticRegression(C=1.0, max_iter=100, random_state=42, n_jobs=1)
clf.fit(X_train_final, train["Course"])
...
print("\n[6] Saving submission...")
out_path = os.path.join(_here, "submission_PROPER_FEATURES_FAST.csv")
